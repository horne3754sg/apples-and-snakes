...
if (!class_exists('AT_Meta_Box')) :
	...
	class AT_Meta_Box
	{
		...
		public function __construct($meta_box)
		{
			...
			if (isset($meta_box['use_with_theme']))
				...
				{
					$this->SelfPath = get_stylesheet_directory_uri() . '/inc/aas-metaboxes';
				}
				else if ($meta_box['use_with_theme'] === false)
				{
					$this->SelfPath = plugins_url('aas-metaboxes', plugin_basename(dirname(__FILE__)));
				}
				else
				{
     ...
     */
		...
		public function show_field_repeater($field, $meta)
		{
			...
			if (count($meta) > 0 && is_array($meta))
			{
				foreach ($meta as $me)
				{
					...
					if ($field['sortable'])
						echo '<span class="re-control"><span class="at_re_sort_handle dashicons dashicons-move"></span></span>';
					
					echo '<div class="title">' . $mmm . '</div>';
					echo '<span class="re-control at-re-toggle"><span class="dashicons dashicons-edit"></span></span>';
					echo '<span class="re-control"><span class="dashicons dashicons-trash" id="remove-' . $field['id'] . '"></span></span>';
					echo '</div>';
					
					echo '<table class="repeater-table" style="display: none;">';
					if ($field['inline'])
					{
			...
			echo '<script>
              ...
              jQuery("#remove-' . $field['id'] . '").live(\'click\', function(e) {
                    e.preventDefault();
                    var del = prompt("Are you sure you want to delete this?");
                    if(del) {
		...
		{
			$this->show_field_begin($field, $meta);
			echo "<textarea class='at-textarea large-text" . (isset($field['class']) ? ' ' . $field['class'] : '') . "' name='{$field['id']}' id='{$field['id']}' " . (isset($field['style']) ? "style='{$field['style']}' " : '') . " cols='40' rows='6'>{$meta}</textarea>";
			$this->show_field_end($field, $meta);
		}
   ...
   */
		...
		public function add_missed_values()
		{
			...
			$this->_meta_box = array_merge(array(
				'context' => 'normal', 'priority' => 'high', 'pages' => array('post')
			...
			foreach ($this->_fields as &$field)
			{
				...
				$field = array_merge(array(
					'multiple' => $multiple, 'std' => $std, 'desc' => '', 'format' => $format, 'validate_func' => ''
				), $field);
			} // End foreach
		...
		{
			$new_field = array(
				'type' => 'text', 'id' => $id, 'std' => '', 'desc' => '', 'style' => '', 'name' => 'Text Field'
			);
			$new_field = array_merge($new_field, $args);
			if (false === $repeater)
			{
		...
		{
			$new_field = array(
				'type' => 'number', 'id' => $id, 'std' => '0', 'desc' => '', 'style' => '', 'name' => 'Number Field',
				'step' => '1', 'min' => '0'
			);
			$new_field = array_merge($new_field, $args);
			if (false === $repeater)
		...
		{
			$new_field = array(
				'type' => 'code', 'id' => $id, 'std' => '', 'desc' => '', 'style' => '',
				'name' => 'Code Editor Field', 'syntax' => 'php', 'theme' => 'defualt'
			);
			$new_field = array_merge($new_field, $args);
			if (false === $repeater)
			{
		...
		{
			$new_field = array(
				'type' => 'hidden', 'id' => $id, 'std' => '', 'desc' => '', 'style' => '', 'name' => 'Text Field'
			);
			$new_field = array_merge($new_field, $args);
			if (false === $repeater)
			{
		...
		{
			$new_field = array(
				'type' => 'checkbox', 'id' => $id, 'std' => '', 'desc' => '', 'style' => '', 'name' => 'Checkbox Field'
			);
			$new_field = array_merge($new_field, $args);
			if (false === $repeater)
			{
		...
		{
			$new_field = array(
				'type' => 'checkbox_list', 'id' => $id, 'std' => '', 'desc' => '', 'style' => '',
				'name' => 'Checkbox List Field', 'options' => $options, 'multiple' => true,
			);
			$new_field = array_merge($new_field, $args);
			if (false === $repeater)
			{
		...
		{
			$new_field = array(
				'type' => 'textarea', 'id' => $id, 'std' => '', 'desc' => '', 'style' => '', 'name' => 'Textarea Field'
			);
			$new_field = array_merge($new_field, $args);
			if (false === $repeater)
			{
		...
		{
			$new_field = array(
				'type' => 'select', 'id' => $id, 'std' => array(), 'desc' => '', 'style' => '',
				'name' => 'Select Field', 'multiple' => false, 'options' => $options
			);
			$new_field = array_merge($new_field, $args);
			if (false === $repeater)
			{
		...
		{
			$new_field = array(
				'type' => 'radio', 'id' => $id, 'std' => array(), 'desc' => '', 'style' => '',
				'name' => 'Radio Field', 'options' => $options
			);
			$new_field = array_merge($new_field, $args);
			if (false === $repeater)
			{
		...
		{
			$new_field = array(
				'type' => 'date', 'id' => $id, 'std' => '', 'desc' => '', 'format' => 'd MM, yy', 'name' => 'Date Field'
			);
			$new_field = array_merge($new_field, $args);
			if (false === $repeater)
			{
		...
		{
			$new_field = array(
				'type' => 'time', 'id' => $id, 'std' => '', 'desc' => '', 'format' => 'hh:mm', 'name' => 'Time Field',
				'ampm' => false
			);
			$new_field = array_merge($new_field, $args);
			if (false === $repeater)
		...
		{
			$new_field = array(
				'type' => 'color', 'id' => $id, 'std' => '', 'desc' => '', 'name' => 'ColorPicker Field'
			);
			$new_field = array_merge($new_field, $args);
			if (false === $repeater)
			{
		...
		{
			$new_field = array(
				'type' => 'image', 'id' => $id, 'desc' => '', 'name' => 'Image Field',
				'std'  => array('id' => '', 'url' => ''), 'multiple' => false
			);
			$new_field = array_merge($new_field, $args);
			if (false === $repeater)
			{
		...
		{
			$new_field = array(
				'type' => 'file', 'id' => $id, 'desc' => '', 'name' => 'File Field', 'multiple' => false,
				'std'  => array('id' => '', 'url' => '')
			);
			$new_field = array_merge($new_field, $args);
			if (false === $repeater)
		...
		{
			$new_field = array(
				'type' => 'wysiwyg', 'id' => $id, 'std' => '', 'desc' => '', 'style' => 'width: 300px; height: 400px',
		...
		public function addTaxonomy($id, $options, $args, $repeater = false)
		{
			...
			$new_field = array(
				'type' => 'taxonomy', 'id' => $id, 'desc' => '', 'name' => 'Taxonomy Field', 'options' => $options
			);
			$new_field = array_merge($new_field, $args);
			if (false === $repeater)
			{
		...
		public function addPosts($id, $options, $args, $repeater = false)
		{
			...
			$new_field = array(
				'type'     => 'posts', 'id' => $id, 'desc' => '', 'name' => 'Posts Field', 'options' => $options,
				'multiple' => false
			);
			$new_field = array_merge($new_field, $args);
			if (false === $repeater)
